...
```text
เมื่อโจทย์อยากให้นับว่าใน input มีสระกี่ตัว คิดง่ายๆ สระก็คือค่าที่เรากำหนดไว้ เราก็สร้างตัวแปรให้มีสระในนั้น เมื่อใส่อะไรมาก็จะนับในข้อความว่ามีสระที่เรากำหนดกี่ตัว ตัวอะไรบ้าง
```
...
1.input
2.ตัวแปรรวมสระ
3. ใช้ for loop เขียน 
for i in (สระ):
count = input.count(i) ให้มันนับหาสระจนกว่าจะหาเจอหมด
เมื่อหาเจอหมดแล้วจึง
if count > 0 
print(f"{i} : {count}")
i คือ สระที่ตรวจเจอซ้ำกับ input 
count คือ จำนวนที่มันเจอซ้ำ
```
...
```text
ไม่มีสระ (output เป็น - เพราะผมไม่ได้กำหนด)
```
