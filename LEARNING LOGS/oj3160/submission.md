...
```text
เขียนโปรแกรมหาจํานวนเฉพาะ พร้อมระบุจํานวนเป็นสองบรรทัดแยก ถ้าไม่มีจำนวนเฉพาะมีแค่บรรทัดเดียว
```
...
```text
รับค่าสองตัวเป็น map split และกำหนด list และทำสมการใน loop
```
...
```text
ทำ loop if i > 1 and all(i % j for j in range(2, int(i**0.5) + 1)) แล้วนำไป append
```
