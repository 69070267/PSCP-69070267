...
```text
โจทย์จะบอกกับเราว่า จะมีแม่สี 3 สี แหละต้องนำสีทั้งหมดนั้นมาใช้เป็นคู่เพื่อที่จะนำมาผสมสีกัน แต่โจทย์กำหนดไว้ว่าจำเป็นจะต้องผสมให้ได้แค่สีที่โจทย์ต้องการเท่านั้นเป็นจำนวน 3 สี ซึ่งจะมี 6 methods (3*2) ผมจึงใช้  if else elif ในการหา

Input :
รับค่าสีที่ต้องการ 2 input โดยที่ต้องการแค่แม่สีเท่านั้น
Output:
แสดงผลสีที่ถูกผสม หากไม่ใช่แม่สี จะเป็น Error และถ้าเป็นสีเดียวกันจะเป็นสีเดิม
Constraints:
ในเมื่อตัวแปรเป็น fixed string เราจึงต้องทำให้ input อะไรมาก็ตามแปลงไปให้เหมือนค่าตัวแปรที่เราตั้งไว้ได้ 
เช่น "       rED " ต้องกลายเป็น "Red" "blue" ต้องกลายเป็น "Blue"
```
...
```text
Step1:รับสีสองสี กำหนดให้รับแล้วตัด spacing และทำให้กลายเป็นตัวพิมพ์ใหญ่ข้างหน้าเท่านั้น
Step2: กำหนดให้ input1 + input2 = output แบบ fixed
Step3:ถ้าใส่ข้อมูลไม่ตรงกับที่ต้องการจะแสดงผลว่า Error และถ้าตรงตามเงื่อนไข จะแสดงผลตาม if elif ที่กำหนดไว้
```
...
```text
จะได้รู้ว่าถ้าใส่แม่สีเหมือนกันจะเป็นอย่างไร
```
...
```text
จะได้รู้ว่าถ้าใส่สีอื่นที่ไม่ใช่แม่สีจะเป็นอย่างไร
```
